...
class Market extends Controller {
	...
	function request($type='food', $from=0, $to=20)
	{
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH']=="XMLHttpRequest")
		{
			...
			$content		= $this->market->get_market($type, $from, $to, $country);
			if ($content->num_rows() > 0)
			{
				foreach ($content->result() as $content)
				{
					$row['price']		= $content->price;
					$$company			= $this->user->data($content->company, 'companies');
		...
		{
			log_message('error', 'User with IP '.$this->input->ip_address().' has tried to enter /market/request doing a XMLHttpRequest.');
			redirect('/market');
		}  
	}
}
